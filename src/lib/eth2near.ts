// NOTE this is mostly copy-pasta from rainbow-bridge cli upstream 

import { loggers } from "winston";

export const bridge = require("rainbow-bridge-lib/eth2near-relay");

// This is a crib of Eth2Near as it is in the CLI
...
function execute(command:any, ..._callback:any):Promise<any> {
  return new Promise((resolve) =>
    exec(command, (error:Error, stdout:any, _stderr:any) => {
      if (error) {
        console.log(error)
      }
      resolve(stdout)
...
export class Eth2NearRelay {
    public ethClientContract:any;
    public robustWeb3:any;
    public web3:any;
    private pause:boolean = false;

  initialize(ethClientContract:any, ethNodeURL:string) {
    // @ts-ignore
    ...
    while (true) {
      ...
      while (true) {
        try {
          ...
          if (clientHashes.find((x:any) => x === chainBlockHash)) {
            break
          } else {
            console.log(
  ...
  async getParseBlock(blockNumber:any) {
    try {
  ...
  async submitBlock(block:any, blockNumber:any) {
        ...
        .map((element:any, index:number) => {
          return {
            ...
            proof: block.merkle_proofs
              ...
              .map((leaf:any) => this.web3.utils.padLeft(leaf, 32)),
          }
        }),
    }
